...
--- MOD_ID: ror2
--- MOD_AUTHOR: [aou]
--- MOD_DESCRIPTION: Adds Benthic Bloom and Egocentrism from Risk of Rain 2 as jokers
----------------------------------------------
------------MOD CODE -------------------------


function SMODS.INIT.ror2()
    local localization = {
        j_benthicbloom = {
            name = "Benthic Bloom",
            text = {
        ...
        j_egocentrism = {
            name = "Egocentrism",
            text = {
                "{X:mult,C:white}X1.5{} Mult. When blind is selected,",
    ...
    for k, v in pairs(jokers) do
        ...
        v.mod = "ror2"
        v:register()
        SMODS.Sprite:new(v.slug, SMODS.findModByID("ror2").path, v.slug..".png", 71, 95, "asset_atli"):register()    
    end
        ...
        if context.setting_blind and not context.blueprint and not self.getting_sliced then
            if pseudorandom('ego') < G.GAME.probabilities.normal/self.ability.extra then
                ...
                for i = 1, #G.jokers.cards do
                    if G.jokers.cards[i] ~= self and G.jokers.cards[i].config.center.rarity ~= 4 and not G.jokers.cards[i].ability.eternal and not G.jokers.cards[i].getting_sliced then deletables[#deletables+1] = G.jokers.cards[i] end
                end
    ...
    SMODS.Jokers.j_benthicbloom.calculate = function(self, context)
        if context.setting_blind and not context.blueprint and context.blind.boss and not self.getting_sliced then
            ...
            if chosen_joker and not self.getting_sliced and not chosen_joker.ability.eternal and not chosen_joker.getting_sliced then 
                ...
                --i am going to hurt someone, just rarity + 1 does NOT work
